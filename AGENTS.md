...
- **原生端手动验收职责**：AI 默认只运行原生端自动化测试（测试、typecheck、lint）并向用户提供走查清单；iOS/Android 模拟器及真机的手动交互、视觉、安全区、系统键盘和读屏验收默认由用户执行并反馈结果。除非用户明确要求，AI 不安装、启动或操作模拟器/真机，也不代替用户生成手动验收结论。需要原生端手动验收的 change，用户反馈通过后方可视为走查完成。
