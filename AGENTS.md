...
- 与用户沟通使用中文。
- **术语约定**：用户所说的「旧项目」指 KnowStruct（`/Users/hujun/Documents/软件/trae/KnowStruct`），
  Grove 的前身产品；其目录共创会话采用 `apply_directory_tree` 工具调用机制（讨论不调用工具、只返回文字，
  仅明确要求修改时调用工具提交完整目标树），并带有文本标记块兜底解析。涉及「旧项目」的讨论默认指它。
