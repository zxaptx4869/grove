...
- [ ] 13.1 建立正式 production adapter + `run_turn` 失败回归，覆盖句柄 9→10、历史 turn 全为 1、旧列表重新展示、隐藏列表不替换位置参照及正文副本一致顺序
- [ ] 13.2 将 direct 与有帮助的 indirect 合并为一个连续标题列表，direct 优先并保留 indirect 说明；删除私有副本、专用激活参数/分支/提示，unrelated 保持排除
- [ ] 13.3 让标题回答误填未读 `discussion_entry_id` 时跳过关联并保留交付；已读对象的权限、范围、对象和指纹失效仍拒绝保存
- [ ] 13.4 补齐“排除上一组”“延续同一讨论方式”“按分析交付完整候选”和已有搜索优先筛选的有界上下文合同，不增加模型阶段、预算、重试或关键词话题路由
- [ ] 13.5 回归统一列表后两条读取、默认标题/明确正文、候选恢复、否定写入声明、正文去重、统计和安全边界，运行相关测试、全量 Ruff、OpenSpec 严格校验和 `git diff --check`
- [ ] 13.6 核对服务与认证；可用时最多一批 12 条消息执行正式 DeepSeek 复测并逐轮语义审阅，不可用时准确记录认证阻断和未覆盖路径
- [ ] 13.7 分段本地提交并将实现、自动化、真实 Run、风险和回退依据写入本地讨论文档，等待用户语义验收且不归档 change
