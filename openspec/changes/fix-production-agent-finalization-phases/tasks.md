...
## 13. 相关结果统一与会话 314 退化修复

- [x] 13.1 建立正式 production adapter + `run_turn` 失败回归，覆盖句柄 9→10、历史 turn 全为 1、旧列表重新展示、隐藏列表不替换位置参照及正文副本一致顺序
- [x] 13.2 将 direct 与有帮助的 indirect 合并为一个连续标题列表，direct 优先并保留 indirect 说明；删除私有副本、专用激活参数/分支/提示，unrelated 保持排除
- [x] 13.3 让标题回答误填未读 `discussion_entry_id` 时跳过关联并保留交付；已读对象的权限、范围、对象和指纹失效仍拒绝保存
- [x] 13.4 补齐“排除上一组”“延续同一讨论方式”“按分析交付完整候选”和已有搜索优先筛选的有界上下文合同，不增加模型阶段、预算、重试或关键词话题路由
- [x] 13.5 回归统一列表后两条读取、默认标题/明确正文、候选恢复、否定写入声明、正文去重、统计和安全边界，运行相关测试、全量 Ruff、OpenSpec 严格校验和 `git diff --check`
- [x] 13.6 核对服务与认证；可用时最多一批 12 条消息执行正式 DeepSeek 复测并逐轮语义审阅，不可用时准确记录认证阻断和未覆盖路径
- [x] 13.7 分段本地提交并将实现、自动化、真实 Run、风险和回退依据写入本地讨论文档，等待用户语义验收且不归档 change

## 14. 相关结果统一与退化修复验证记录

- 2026-09-18 实现：语义筛选仍由同一 Agent 对真实候选标注 `direct` / `indirect` / `unrelated`，程序将前两者按 direct 优先合成唯一授权标题列表，排除 unrelated，并在结构化列表后交付间接关联说明。删除 `internal_classified_items`、`relevance_scope=indirect` 激活参数、隐藏集合快照特例及对应提示/分支；旧快照仅保留现有记录格式的兼容渲染。
- 2026-09-18 顺序与讨论：最近结果以 `history_turns` 数组顺序和最终 blocks 引用为准，兼容历史 `turn=1`；句柄数字只在同一实际交付时破同序，内部未交付结果不替换序号参照。标题回答多填未读 `discussion_entry_id` 时记录 `discussion_association_not_read` 并跳过关联；已读对象缺少校验引用、授权/发现失效或指纹变化仍拒绝保存。
- 2026-09-18 自动化：`test_knowledge_agent_*.py` 单进程串行 500 项全通过；其中共享循环 176 项、production adapter 19 项分别全通过。正式 adapter + 真实 `run_turn` 夹具覆盖 13 个跨 Run 位置参照（句柄跨 9→10）、统一相关列表后两条正文读取、finalizer 可选讨论关联跳过、“第二条也分析”、完整候选输入及独立新问题。全量 Ruff 通过，OpenSpec 严格校验 64 项通过、0 失败，`git diff --check` 通过。两次误将共享 SQLite 夹具并行运行导致 `readonly database` / `disk I/O error`，串行重跑全通过，该并发夹具问题不计为业务失败。
- 2026-09-18 服务与真实复测：`uvicorn --reload` 子进程于 16:14:12 启动，与 `loop.py` 最后修改时间一致，且正在 `127.0.0.1:8000` 监听，可确认本轮后端代码已 reload。零模型 preflight 在任何 API 或模型请求前因非交互环境未找到已保存 demo 凭据而退出；本轮新增真实批次、会话、Run、用户消息和模型调用均为 0。会话 314 / Run 886–901 仅作为修复前证据，新行为的 DeepSeek 语义验收仍未覆盖，change 不归档。
