...
工具结果的 `displayable` 只表示内容通过结构与授权检查、允许进入回答，不等于用户已经看到。循环在保存历史时从最终 `blocks` 提取实际交付的结果句柄和父集合顺序；跨 Run 的“第一条”“第二条”及最近展示组只使用这些交付引用。当前轮尚未交付的工具结果仍可供 Agent 继续读取或筛选，但不得静默替换上一轮用户可见的顺序。

同一窗口存在多个已交付列表且用户没有提供足够指向时，由 Agent 请求澄清；不增加位置关键词路由。新列表只有在最终回答引用后才成为较新的位置参照。Workspace、权限、项目范围、对象存在性和指纹复验保持不变。

### 14. 间接相关集合保留分类并按请求激活

相关性筛选继续默认只把直接相关标题列表交付给用户，同时将同次筛选的间接相关对象保存为有界、不可默认展示的已分类集合。Agent 只有在用户明确要求查看该集合时，才通过既有工具合同激活它；激活前逐项复验 Workspace、权限、项目范围、对象存在性和材料指纹，且最终仍只展示标题，不自动读取正文。

该路径复用原筛选结果，不重新搜索同一宽泛问题，不把 `indirect` 改报为 `direct`，也不把内部已有材料丢失描述成用户缺少线索。它不新增模型阶段、任务路由或通用记忆系统。
