...
### Requirement: 位置指代必须依据实际交付的列表

系统 MUST 区分允许展示的工具结果与最终回答实际交付的结果。跨 Run 的位置指代和最近展示顺序 MUST 只由最终 `blocks` 引用的列表确认；内部搜索、筛选或读取产生但未交付的列表 MUST NOT 静默替换用户的位置参照。新列表实际交付后 MUST 成为新的可引用顺序。存在多个已交付集合且无法唯一判断时 MUST 澄清，并继续执行 Workspace、权限、范围、对象存在性和指纹复验。
...
### Requirement: 明确请求时必须可承接已分类的间接相关记录

相关性筛选 MUST 保持 `direct` 与 `indirect` 的真实分类，默认回答 MUST 只优先展示直接相关标题且不得自动展开正文。同次筛选的间接相关集合 MUST 有界保留；用户明确要求查看时，系统 MUST 能复用该集合并在交付前逐项复验 Workspace、权限、项目范围、对象存在性和材料指纹，不得要求用户重新提供系统已经持有的线索，也不得通过重新执行同一宽泛搜索碰运气。

#### Scenario: 默认只展示直接相关标题

- **WHEN** 一次筛选同时得到直接和间接相关记录，而用户尚未要求查看间接记录
- **THEN** 最终回答只展示直接相关标题及准确的间接数量，不自动展示间接标题或正文

#### Scenario: 用户随后要求展示间接相关记录

- **WHEN** 用户明确要求查看刚才的间接相关记录
- **THEN** Agent 通过既有筛选展示合同激活原 `indirect` 集合，复验安全边界后按原分类展示标题
- **AND** 不把间接记录改报为直接相关，不重新搜索同一问题，也不声称用户缺少系统已经持有的信息

#### Scenario: 间接集合安全条件失效

- **WHEN** 激活前权限被撤回、项目范围变化、对象删除或材料指纹变化
- **THEN** 系统拒绝展示失效对象，不用旧分类结果绕过安全检查
