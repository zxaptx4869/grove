...
统一循环允许同一模型响应提出多个工具，但实验副本中的“只读”工具仍会写审计或 Evidence。工作台为创建对话、提交 Run、各工具数据库事务和 Run 终态写入复用同一异步锁，只串行 SQLite 事务，不提高 busy timeout、不重试模型，也不改变共享领域工具算法。这样避免多个延迟读事务同时升级为写事务，同时保留模型请求和已冻结预算语义。

`run_turn` 形成的公开回答、工具记录、模型调用和 usage 先保存在内存结果中；`_finish_new_run` 失败时以独立 `persistence` 诊断附加到同一轮并落本地 JSON，不再由工作台普通异常分支覆盖。求解状态与 Run 保存状态分别展示，缺失的调用记录保留为未知。

业务指纹仍同时核对原库和副本，但同步 SQLite 查询通过线程执行，使等待锁时事件循环仍能完成连接回滚与取消清理。指纹不一致标记 `changed`；查询异常标记 `failed`，仅表示无法核验。两者都会停止后续数据库操作，最后的本地记录保存不依赖该检查成功。
