...
- 为已知目录名或完整路径增加直接定位操作，返回真实 Node 身份、完整路径、深度、同名候选和完整性；精确名称优先，不把模糊匹配冒充唯一结果。
- 允许统一循环从目录结果句柄解析真实 `node_id`，再按该节点直接范围或子树范围查询正式 Entry；后续追问复用已定位句柄，不重复从项目根遍历。
- 补充无真实模型调用的领域查询、循环接线、历史引用、权限隔离、失败续聊和既有混合结果回归测试；不重放历史消息、不调用付费评测。

非目标：目录创建/移动/删除、旧 Agent 正式流程替换、生产接入、取消诊断补全、搜索回答质量改造、预算提高、历史压缩重构或新增意图/规划模型。
...
- 后端：`backend/app/services/knowledge_agent/` 的共享只读工具合同、目录定位和 Node 范围 Entry 查询；`backend/evals/dialogue_loop/` 的工具暴露、结果合同、停止状态、续查快照、历史摘要和确定性渲染。
