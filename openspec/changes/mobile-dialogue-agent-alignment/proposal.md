...
- 冷启动在没有移动端未完成工作时默认进入空白新对话；短暂前后台或栏目往返保留当前会话、输入与阅读位置。必要的恢复引用按账号和 Workspace 隔离持久化，并由现有服务端 API 复验。
- 历史 Sheet 使用虚拟列表减少首屏渲染量，新建入口不依赖历史请求；移动端回答、候选与知识正文使用纯原生 Markdown 展示并提供文本选择。
...
- `native-knowledge-agent-conversation`：收敛提问设置，增加正式 dialogue 状态、continuation、启动恢复与三类重试语义，明确顶部/历史入口、默认新对话、提交进行中/结果未知和首次定位行为，删除旧模式覆盖要求。
- `native-knowledge-agent-answer`：以正式 dialogue blocks 展示回答与真实依据，使用紧凑真实阶段、轻量取消入口和原生 Markdown，移除旧 citations、Candidate Draft、Entry Revision 和复杂旧回答合同。
- `native-knowledge-agent-entry-results`：将正式块中的知识列表按服务端原序紧凑展示，移除正文摘要，并支持明确 Entry 的当前正文按需底部详情与复制阅读。
...
- 不新增数据库迁移、模型调用或 Agent 编排阶段；移动端新增一个与当前 React Native 版本兼容的纯 JavaScript Markdown 渲染依赖。
