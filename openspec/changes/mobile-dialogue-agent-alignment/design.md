...
- [Sheet 切换为滑入影响其他弹层或首帧闪动] → 只给 Entry 详情传入局部动画类型，通用 Sheet 默认仍为 fade；详情按当前单条内容自适应且限制最大高度，短时读取完成前不呈现 Modal，慢读取超过短暂阈值才展示 loading；底部 Modal 启用 Android 硬件加速，布局提交前写入隐藏起点并在下一帧启动滑入；减少动态效果时详情使用 none。
