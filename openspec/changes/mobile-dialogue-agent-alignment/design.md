...
### 15. 发送反馈与键盘避让保持单一布局变化

发送中的本地消息与服务端确认后的正式用户消息按同一 `client_message_id` 使用稳定 React key 和相同气泡几何，只在固定状态槽表达发送中；成功确认时不卸载再挂载整条消息，不用整体透明度或额外标签改变高度。点击发送仅标记下一次内容布局需要跟随，不在消息尚未插入时预先滚动；pending 气泡完成布局后滚动一次，后续近底部内容变化使用无动画锚定，避免连续滚动动画竞争。

Android 已配置 `softwareKeyboardLayoutMode=resize`，因此系统缩小可用高度是唯一键盘避让负责人，页面不得再给 Composer 叠加完整键盘高度。iOS 继续使用键盘事件高度作为输入区 inset。键盘从关闭变为打开时，页面将当前对话定位到最新内容；关闭键盘、轮询更新、详情关闭和普通重渲染不重复触发该定位，加载旧历史的高度锚点不变。
...
- [Sheet 切换为滑入影响其他弹层或首帧闪动] → 只给 Entry 详情传入局部动画类型，通用 Sheet 默认仍为 fade；详情按当前单条内容自适应且限制最大高度。底部面板以独立透明度保持隐藏，同时等待原生 Modal `onShow` 与面板有效 `onLayout`，两者均到达后才按实测面板高度加阴影余量设置屏外起点并启动一次入场；打开代次使关闭、重开和迟到回调相互隔离。Entry 展示组件在入场开始时保存 loading/正文/错误快照，入场完成后才以局部 `LayoutAnimation` 接入真实变化；没有延迟查询结果时，完成回调只更新 ref，不在动画收尾帧提交 React 状态。详情根组件通过 memo 与会话轮询隔离，并稳定关闭和重试回调，避免无关父级更新重复协调详情树。140ms 只决定快请求直接呈现正文还是慢请求提供可关闭 loading，不参与动画就绪判断；减少动态效果在同一双就绪门控后直接显示且不因偏好迟到而重播，也跳过内容布局过渡。
...
- [Markdown 扩大富文本输入面且原生列表可能按内容固有宽度溢出] → 禁用 HTML 和图片，限制链接协议，结构化块边界不变；根容器、列表行和正文列显式约束可视宽度并允许收缩换行，只有表格与代码使用横向阅读回退。
