...
- 自动路由在复合能力开启时使用原始问题，解释 + 来源 + 统计优先 quick；只有确需多轮冲突核验才 investigate。显式 investigate 保持现有调查流程，本次不迁移其工具能力，不能声称该模式已支持结构化统计。
- 回答要点增加可选内部“直接回答核心问题”标记；只对通过引用校验且被明确评估为直接回答的要点，纠正互相矛盾的 insufficient 为 partial，不把任意边缘证据当作已回答。Run 同步反映 answer 的非完整状态。
- 模型知识使用按最终保留内容派生，不再以允许使用代替实际使用；原生端不在知识不足默认文案中推断模型依据，状态徽标与依据标签分开表达。
- 复合 point 标题使用已冻结义务摘要，同义务相同正文去重；跨请求复用的 Evidence 只输入模型一次。提示明确不以环保等级等旁支替代来源等当前义务，但不声称确定性代码能验证任意自然语言相关性。
- 复合模型输出收窄为 points/conflicts，去除由服务端派生的重复全局状态字段；普通 quick/investigate 的历史草稿仍兼容。补查 prompt 升为 v2，已冻结 v1 计划仍可恢复，不因升级重新规划。
- 补查 planner 接收已执行查询、结构化过滤与结果摘要；同义务结构化补查不得修改类型、性质和时间过滤，语义改写仍受允许。不同有限匹配结果不能相加、取最大值或冒充全量总数；展示首次/补查阶段和有限集合边界。
- 再综合失败只保留首次合法正文，可附加服务端确定性统计；新的展示材料存入 final_result，恢复时不再次综合。失败提示在正文前，公开 gaps 标明综合未完成，不丢失原始快照。
- 新增展示材料同样受冻结快照字节预算限制；若容纳不下，则回退完整首次结果并保留失败提示，不截断引用或扩大预算。
- 实际验收 Run 144 属于项目会话 70，Run 145 属于 Workspace 新会话 71，范围变化不是执行器扩大范围；保持新会话默认 Workspace 的既有语义，补充范围回归与复验提示。
