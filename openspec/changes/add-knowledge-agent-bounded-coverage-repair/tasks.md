...
- [x] 7.2 修复自动复合路由、部分回答与 Run 状态、模型依据派生并增加回归。
- [x] 7.3 修复复合正文分组去重、补查统计口径、失败确定性事实保留和恢复回归。
- [x] 7.4 原生端修复不足文案、状态徽标、补查失败前置提示，补充范围和组件回归。
- [x] 7.5 执行后端与原生端自动化检查，记录八项复验结论并本地提交；真机走查仍待用户反馈，不归档、不推送。结果见 `docs/验收记录/add-knowledge-agent-bounded-coverage-repair-fixes.md`：后端 799 项、原生端 148 项通过。
