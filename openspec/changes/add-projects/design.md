...
- 装修模板种子：创建项目时可选模板，注册时自动建「房子装修」示例项目。
...
### D3：多项目模型，注册自动建示例项目
`Project(workspace_id, name, template)`；注册时自动创建「房子装修」项目（decoration 模板），让首个场景开箱即用；用户可重命名、删除或新建更多项目（v1 UI 支持多项目列表）。

**备选**：注册只建空工作区、首次手动建项目——更「干净」，但自用首场景多一步；按用户「装修只是案例」的定位，示例项目可随时删除，选自动创建。
...
- [自动建示例项目被认为是噪音] → 可删除/重命名；决策已记录，用户可随时调整。
...
- 无阻塞项。「注册自动建示例项目」如需改为空工作区，实施前可调整。
