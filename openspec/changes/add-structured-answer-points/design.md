...
要点卡结构（对齐原型 `.answer-points`，正式组件重写）：

- 分组标题：`section` 变化时渲染一行加粗标题（可带 `▍` 前缀符号）；
- 编号：全回答连续 `1..N`，左侧绿色圆底数字（对齐原型 `point-number`）；
- 正文：每条一个 `Text`；
- 来源 chip：**放在要点正文下方一行**（`minHeight 44`），复用现有 `onCitationPress` → `CitationSheet`；这是对原型「行内按钮」的有意偏离——行内嵌按钮在 React Native 中触控区域与长文本换行都难处理，且 Grove 验收要求 44×44 触控目标；
- 读屏：chip `accessibilityLabel="查看引用：{entryTitle}"`；编号与分组标题对读屏保持可读文本；
- 底部 `CitationStrip` 在 `points` 存在时保留为「全部来源速览」（与原型一致）。
