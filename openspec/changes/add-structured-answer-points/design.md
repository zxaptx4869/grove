...
### 5. 原生要点卡渲染与触控（按用户反馈修订为「上标序号 + 底部来源映射」）

真机反馈：当前「每条要点下方一个来源 chip」在「一条要点 = 一条 Entry」的回答里，
来源标题与要点内容高度重复，显得多余；用户收集知识后用 AI 问是为了快速得到综合结论，
来源应「按需可查」而非「每条贴脸」。改为方案 B：

- 分组标题：`section` 变化时渲染一行加粗标题（带 `▍` 前缀符号）；
- 每条要点：正文后跟一个**极小上标序号**（`① ② …`，同一要点多条引用时并列多个），
  不再渲染逐条来源 chip；
- 底部来源区：按序号列出「序号 + 来源标题」（`minHeight 44` 可点击行），点击复用现有
  `onCitationPress` → `CitationSheet`；有 `points` 时该来源区替代底部 `CitationStrip`
  「全部来源速览」，避免两套来源入口重复；
- 序号规则：按 `points` 顺序对全部引用去重编号（同一 Evidence 只占一个序号）；每条要点
  显示其引用对应的序号上标；超过 20 条时序号回退为 `(n)` 文本；
- 读屏：来源区每行 `accessibilityLabel="查看引用 {序号}：{entryTitle}"`；上标序号对读屏
  保持可读文本；
