...
- 用户可见列表（语义搜索 / 相似推荐）最终输出加余弦下限 0.3：低于下限的弱相关条目不展示；余弦未知（向量未就绪）的条目保留。下限只作用于用户可见列表，Reader 上下文与关系判断不套用。重排 prompt 同步收紧：「同主题优先、跨主题仅保留强相关、宁可少给」，解决阈值切不干净的跨域弱相关（真实样本中相关与无关条目分数带重叠，单一阈值无法分离）。
...
模型配置交互（2026-08-26 验证期补充）：三张模型卡改为「展示态 / 编辑态」，改动必须显式保存才生效；保存时若 embedding 模型名发生变更，弹窗告知「切换语义模型将全量重建」及原因（不同模型向量无法混用），确认后由后端自动删除旧向量并按新模型重建，避免旧模型残留与重复行。`mark_entry_embedding_pending` 与全量重建均保证同一条目只保留当前模型一行。
