...
- [x] 5.2 运行后端测试与静态检查
- [x] 5.3 运行前端测试与构建
- [x] 5.4 手动走查目录共创全流程（澄清 → 候选树 → 对话调整 → 内联编辑 → 应用）
...
- [x] 6.11 草稿节点增加 `selected`（默认采用），前端默认展示态 + 勾选 + 编辑按钮，应用只创建勾选节点
- [x] 6.12 等待生成超过 20 秒时显示更明显的提示
- [x] 6.13 移除项目首页与知识空间页头的 AI 共创入口，仅空目录内容区保留
- [x] 6.14 澄清支持第二轮问题（批次上限 2），提交答案后再判断
- [x] 6.15 失败草稿重启时清零会话轮数
- [x] 6.16 失败态提供“重新发起”，对话显示“AI 思考中…”，env 模板补 Worker 开关
