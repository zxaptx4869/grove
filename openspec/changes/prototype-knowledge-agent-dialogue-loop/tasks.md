...
- [x] 9.1 为实验入口增加系统钥匙串读取、显式保存与删除；保存前在隔离副本验证 demo 身份，缺失或读取失败回退隐藏输入，不记录密码。
- [x] 9.2 使用内存 SecretStore 覆盖读取、回退、验证后保存和删除路径；更新 README，执行相关无模型检查和严格 OpenSpec 校验后本地提交。

## 10. 第一批稳定性修复

- [x] 10.1 将工具、向量、Entry/Evidence、文本、输入长度、时间和整批预算的可恢复停止统一接入最多一次无工具收尾，不增加或重置预算。
- [x] 10.2 使用离线 FunctionModel 和临时 SQLite 副本覆盖并行工具事务、预算停止、取消及安全材料保留；不启动新的真实评测。

### 第一批稳定性验证（2026-09-07）

- FunctionModel 回归覆盖工具动作、单轮／整批向量、Entry、Evidence、文本、输入软／硬上限、求解时间和整批文本停止；可恢复停止只进入一次无工具收尾，输入或请求无额度时为 `not_dispatched`，收尾失败、超时和非法输出不重试，取消直接传播。
- 失败轮次从调用日志恢复整轮 usage；任何已派发请求缺失 usage 时 token 与费用保持未知，并另记已知请求数，不能以零替代。
- 未启动新付费评测，未重发历史失败消息，目录读取能力、记录类型解释、统计标题、检索排序及回答提示词策略均未修改。

### 实验 demo 凭据验证

- 在提交 `37c7167` 上通过显式保存命令隐藏读取密码，隔离身份校验成功后写入当前 demo
  Workspace 的 macOS 钥匙串；随后直接执行无模型预检，命令确认从钥匙串读取且没有再次等待输入。
- 聚焦无模型回归共 54 项通过，覆盖 Workspace 键隔离、已有密码不询问、缺失／读取失败回退、
  安全错误文本、空密码拒绝、删除、命令互斥，以及隔离认证失败时不写钥匙串；测试只使用内存存储。
- 无交互预检报告：
  `backend/data/knowledge-agent-evals/dialogue-loop/prototype-v2-20260907-173443/report.md`。
  两臂预检通过且模型请求为 0，原库及副本业务指纹不变；报告目录权限为 700、文件权限为 600，
  敏感模式扫描无命中。本次没有启动真实评测。
