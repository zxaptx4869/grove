...
- [ ] 8.1 将完整 Grove 规则改为每次 run 的当前 instructions，并清除历史旧 system；用实际 FunctionModel 请求覆盖首轮、续聊、结构化纠正和独立收尾，确认不重复注入。
- [ ] 8.2 为文本调用记录有界公开响应、结构化调用参数、finish reason、错误类别和异常链；区分 schema、引用、截断、预算和 Provider 错误，不记录隐藏推理或敏感配置。
- [ ] 8.3 将输入估算改为实际 Provider 请求形状投影，记录组件、估算版本、调用来源和已派发请求误差；保持 9000／12000 token 边界及一次收尾预算。
