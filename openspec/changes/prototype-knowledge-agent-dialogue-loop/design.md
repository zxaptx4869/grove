...
每组不超过四轮，先保留完整的有界对话和工具摘要，不实现长期摘要或复杂任务分支状态机。输出超长时裁剪工具正文并标明截断，保留调用与结果配对、明确限制和对象引用；超过模型输入预算则明确停止，不悄悄删掉用户条件。
...
白名单覆盖授权项目枚举／解析、语义搜索、结构化筛选排序、精确聚合、Entry 正文与来源读取、列表位置解析。项目枚举直接使用受成员关系约束的服务端查询；其余优先复用 `read_tools.py`、`read_tool_adapters.py`、`structured_query_tools.py` 与 `tools.py`，不暴露任意 SQL 或写工具。
...
当前只生成规划工件并严格校验、本地提交。获批后先实现隔离预检与无模型测试，再实现循环与固定对照；真实评测只在其范围获批后启动。实验完成无论效果好坏都报告证据，等待用户审阅后按 AGENTS.md 收尾，不自动推送、合并或归档。

有效代码后续选择性迁入基础开发分支，不将本实验分支直接作为正式版本发布。删除或停用实验入口即可停止实验，不需要生产数据回滚。

## Open Questions

待用户审阅的是本提案整体范围与第 6 节预算；当前没有阻止提案完成的缺失信息。输入／输出长度的具体配置由实施预检结合已安装 Provider 能力固定，无法执行共享预算时停止并报告，不自行放宽。长期历史压缩、正式状态迁移和 UI 块协议属于后续正式改造。
