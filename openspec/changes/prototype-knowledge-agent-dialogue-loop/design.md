...
身份验证复用应用认证逻辑，使用既有 demo 账号。实验提供显式保存命令：密码经隐藏输入并在隔离副本验证成功后，使用应用已有 `SecretStore` 写入按 Workspace 命名的系统钥匙串项；正常运行优先读取该项，缺失或钥匙串读取失败才隐藏询问，并提供显式删除命令。密码不写命令参数、环境变量、仓库或报告，测试只使用内存密钥存储；钥匙串错误只输出类型与安全说明，不输出密码。身份、Workspace 成员关系和界面范围从服务端验证结果取得，不能从模型或测试预期取得。Provider 继续通过应用配置读取，不把 API key 复制进脚本或输出。临时副本可能含敏感配置，因此目录权限 700、文件 600，退出清理副本；异常退出后的残留只在下一次运行时清理本实验自有目录。
...
工具、向量、Entry/Evidence、文本、输入长度和整批预算统一抛出可识别的 `BudgetExceeded` 停止信号。统一循环记录求解失败后进入同一个 `_finalize_once`：最多一次、禁用工具、独立 15 秒窗口，同时受单轮与整批文本预算及输入硬上限约束。没有额度或输入仍超限时只记录 `not_dispatched`；取消、权限与隔离失败不进入该分支。

同一响应中的工具任务继续由框架负责失败即取消并 drain；实验 `LoopState` 额外持有数据库事务锁，确保 SQLite 副本上的业务读取与审计／Evidence 写入按事务串行。该锁与工作台 Run 生命周期使用同一实例，终态保存不会和遗留工具事务竞争；不修改共享服务、不改变正式 App 的数据库并发策略。
