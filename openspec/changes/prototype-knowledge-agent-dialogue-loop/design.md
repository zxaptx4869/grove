...
第二版保持第一版模型、每请求输入上限 12,000 token、输出上限和表中请求总额，便于比较修订效果。输入长度采用与模型协议相容的确定性估算并设置低于 12,000 的资料软阈值，至少预留一次收尾请求所需空间；具体估算阈值在预检和报告中冻结。真实 usage 仍只采用 Provider／框架返回值，不能由估算替代。第二版真实评测不在本次实施授权内。
...
第二版规划修订、代码实现和无模型验证已获批准；下一批真实评测尚待用户确认。保持第一版模型与 12,000 token 输入上限，除非无模型证据证明无法实现预算内收尾；如必须调整，先报告证据和可比性影响。长期历史压缩、正式状态迁移和 UI 块协议属于后续正式改造。
