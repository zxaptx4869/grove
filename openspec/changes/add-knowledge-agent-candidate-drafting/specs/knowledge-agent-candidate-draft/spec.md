...
系统 MUST 只接受锚定同一用户、Workspace 与 Conversation 内 `completed` 或 `partial` 回答 Run 的显式 `draft_candidate` 请求；来源回答 MUST 至少有一条最终有效 citation。普通问答消息 MUST NOT 因文本看似包含保存或修改意图而自动进入写分支。

#### Scenario: 从有证据回答发起
- **WHEN** 用户从一条有最终有效 citation 的 completed 回答点击「整理成知识」
- **THEN** 系统保存可见用户消息并创建锚定该 source Run 的候选草稿 operation Run

#### Scenario: 部分回答只整理可确认部分
- **WHEN** partial 回答仍包含有效 citation 且用户显式发起整理
- **THEN** 系统允许生成草稿，但只向生成器提供最终有效 Evidence，不把未解决 gap 当作事实

#### Scenario: 知识不足或无引用回答
- **WHEN** 回答为 insufficient、failed、clarification、cancelled 或没有最终有效 citation
- **THEN** 系统不提供可执行整理动作，伪造请求被拒绝且不创建 Draft
