...
系统 MUST 在范围变更时记录范围事件并更新对话当前范围；已有消息与 Run MUST 保留生成时的范围快照，不得因后续范围切换而改变。

#### Scenario: 空闲对话切换范围
- **WHEN** 用户在没有活动 Run 的对话中从 Workspace 切换到某个项目
- **THEN** 系统更新当前范围并追加包含前后范围标签的 `scope_change` 系统消息

#### Scenario: 活动 Run 期间切换范围
- **WHEN** 对话存在 `waiting` 或 `processing` Run 且用户请求切换范围
- **THEN** 系统返回冲突响应且保持原范围不变

#### Scenario: 查看历史消息范围
- **WHEN** 用户切换范围后读取切换前的消息与 Run
- **THEN** 系统仍返回它们生成时的 Workspace 或项目范围快照

### Requirement: 对话与消息历史可恢复
系统 MUST 持久化用户消息、助手消息和范围事件，并提供按当前 Workspace 列出对话、按游标分页读取消息及恢复最近活动状态的接口。

#### Scenario: 重新打开对话
- **WHEN** 客户端重启后读取一个有权限的对话
- **THEN** 系统返回当前范围、持久化消息及其关联 Run 状态
...
系统 MUST 要求客户端提交稳定的 `client_message_id`；同一对话内重复提交相同标识 MUST 返回首次创建的用户消息和 Run，不得再次执行问答。

#### Scenario: 首次提交问题
- **WHEN** 用户以新的 `client_message_id` 向空闲对话提交非空问题
- **THEN** 系统在同一事务中创建用户消息、助手占位消息与待执行 Run

#### Scenario: 网络重试重复提交
- **WHEN** 客户端在同一对话重复提交已使用的 `client_message_id`
- **THEN** 系统返回原用户消息和原 Run 且不创建重复记录
