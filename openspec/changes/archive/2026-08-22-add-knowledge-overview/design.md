...
- Code Review P2 已修复：钻取时重置视图到完整画布，避免缩放/平移状态下钻到视口外；
- Code Review P3 已修复：扇区支持键盘操作（tabIndex + role + Enter/Space）、hover/mousemove 不再全量重建扇区（memo）、扇区上限只统计 path；
- 剩余 P3：双模式常驻挂载（保留状态的设计取舍）；`MindMapView` 独立模式为兼容历史直链的死代码（已加注释）；视觉验收（1280 / 1440 / 1600 截图）暂缓，待用户确认后补充。
