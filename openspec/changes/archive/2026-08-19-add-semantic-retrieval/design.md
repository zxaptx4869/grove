...
### 决策 8：召回质量验证标注集留到验证阶段

真实标注集依赖真实装修数据与人工的语义相关性判断，属于产品验证阶段的活，不在本次功能 change 内产出。本次 change 的后端测试覆盖确定性召回逻辑与降级路径，保证功能正确；`Recall@10` / `Precision@10` 的量化验证留待验证阶段结合真实数据集进行。

### 决策 9：未配置密钥时的 fallback 按召回分数降序

未配置文本模型密钥时，语义检索降级为确定性召回结果，排序按召回分数（bigram / Jaccard 相似度）降序，保持「相关度优先」的排序语义与正常路径一致；结果显式标记 `is_fallback`。
