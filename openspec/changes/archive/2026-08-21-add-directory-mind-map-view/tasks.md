...
- [x] 7.4 画布背景经点阵尝试后定为原型式线格网格（`mind-map-grid`），节点层次微调；决策记录在 design.md
- [x] 7.5 更新 MindMapView 测试（项目总根、裁剪、项目 Entry）并全量回归

## 8. 交互升级：知识进入画布

- [x] 8.1 目录节点提供「+N 直接知识」展开入口，展开后该节点直接 Entry 以缩略小卡形式作为终端分支展示在画布中
- [x] 8.2 点击知识小卡或侧栏条目后，阅读侧栏展示 Entry 详情（全文、类型、来源证据），并提供返回列表
- [x] 8.3 画布背景改为原型式线格网格，并固定在滚动容器上不随内容滚动
- [x] 8.4 更新 MindMapView 测试（知识展开/收起、小卡详情、背景与裁剪）并全量回归

## 9. 交互定稿：点击展开 + 悬浮详情

- [x] 9.1 点击目录节点切换直接知识展开：叶子节点右侧列堆叠、非叶子节点同列下方堆叠，布局预留行位整体下推
- [x] 9.2 知识详情改为悬浮小窗：hover 预览 + 点击固定 + 可关闭，画布小卡与侧栏条目共用（EntryPopover）
- [x] 9.3 移除「+N」展开按钮与侧栏详情模式，侧栏回归节点知识列表
- [x] 9.4 更新 MindMapView 测试（点击展开/收起、hover 预览、点击固定、列表悬浮详情）并全量回归
- [x] 9.5 Code Review 修复：仅在节点有直接知识时切换展开，避免项目总根/空节点发起无效请求
- [x] 9.6 Code Review 清理：移出旭日图探索原型代码（路由/组件/壳条件），正式分支仅保留规格内功能
