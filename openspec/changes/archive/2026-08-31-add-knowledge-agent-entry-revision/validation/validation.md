...
审查后修复（2026-08-31 二轮审查）：

1. 确认/撤销乐观并发原子化：校验指纹/版本前对目标 Entry 行加
   `SELECT ... FOR UPDATE` 锁定，关闭「校验通过后、写入前」被其他会话修改的
   覆盖窗口；撤销同时复验项目仍属于同一 Workspace。SQLite 下 FOR UPDATE 被
   方言忽略（全库写锁本身串行），MySQL 并发语义仍列为真机/压测未验证项。
2. 可观测性语义修正：非模型失败（证据失效、无实际差异）不再写入
   has_fallback 降级标记，避免监控把任务失败误判为模型降级；模型失败时
   provider/model 随 meta 一并记录。
3. 冲突文案与引导：确认幂等/并发冲突 409 文案中性化（不再声称“已进入终态”）；
   确认 Sheet 的确定性冲突按场景给出「取消并重新发起」或「编辑后重试」引导。
4. 修订草稿卡「目标」行不再重复展示草稿标题，改为单条正式知识语义说明。
5. 撤销删除本操作新增 Evidence 时按实际 rowcount 记录移除数量。
