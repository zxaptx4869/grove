...
- 增加适配层的确定性测试和短链路 API 验证入口，并完成真实模型人工验收。
- 在 Grove Web 应用壳中接入正式 Conversation、Message、Run API，提供桌面对话薄壳；页面复用统一循环结果，不接入实验台本地服务。

### Non-Goals

- 本阶段只实现 Grove Web 桌面对话薄壳，不实现移动端页面或独立实验台页面。
...
- 前端：在现有 Grove 应用壳增加知识 Agent 入口和对话页，复用正式 API 与结果合同；不复制实验台 JSON 存储，并移除右侧运行状态面板。
