...
## 15. 2026-09-18 归档前独立核对（尚未归档）

- 用户自行完成正式会话 316（Run 919–935），17 轮为 16 completed、1 partial；Run 932 搜索后触发软阈值，Run 933 继续完成筛选及回答。相关结果统一、间接项直接读取和两位数句柄的位置引用有真实成功证据，不把 completed 数量等同于语义通过率。
- 以当前代码离线重放会话 315 的 Run 908、916 快照，原失败位置现分别解析为 Entry 35、91；Run 918 原输出不再因未读正文的可选 discussion ID 拒绝整轮，而是记录跳过关联诊断。
- 会话 316 的 Run 926 仍未向最终回答阶段传递上一轮“抛开知识库”的讨论方式；Run 927 已拿到分析但仍偏原文加点评。用户已明确暂缓实施，并同意在本地优化清单第 53、54 项登记。这两项不记为真实语义验收通过。
- 最新真实会话未覆盖“除了这些”的排除追问、连续口语化候选改写及候选失败恢复；任务 8.1、8.2 保留未完成状态，不用部分人工样本冒充完整评测报告。
- 归档前扩大到知识 Agent 与实验台全部相关回归，622 项中仅一项失败：实验台旧反例仍要求禁止 indirect 标题，与已批准的统一相关结果合同冲突。仅将该反例分类改为 unrelated 并更新预期错误文案，未修改业务代码。修正后所在文件与共享循环 197 项全部通过，其余 621 项在前次检查通过。
- 前端 30 个测试文件 / 149 项通过，生产构建通过；ESLint 为 0 错误、2 条既有 DirectoryDraftDialog Hook dependency 警告。后端全量 Ruff、OpenSpec 严格校验 64 项与 git diff --check 通过。
- 用户授权的目标是归档当前 change、合并 main 并推送，同时保留远端 codex/fix-production-agent-finalization-phases。当前 main 与 fetch 后的 origin/main 均为 a76da96，无其他主线增量。尚未归档、合并、推送或部署；任务缺口与例外收尾边界须在实际归档前明确，不能声称全部语义目标完成。


## 16. 2026-09-18 用户批准的例外关闭

- 用户在阅读第 15 节对应的独立核对结论后，明确回复“OK，没问题，继续吧”，批准保留验收缺口并例外关闭本 change、同步规格、归档、合并 main 和推送；远端特性分支必须保留。
- 本次关闭不是“所有语义目标均验收通过”：任务 8.1、8.2 仍未完成，未补跑真实模型，也不将未自然覆盖的排除追问、连续候选改写和候选失败恢复标为通过。
- 两项已知限制继续暂缓：同组跨对象追问未稳定传递讨论方式至 finalizer；已收到分析的候选交付仍可能表现为原文加点评。用户已同意登记本地优化清单第 53、54 项，后续独立处理。
- 规格同步保留目标行为与安全合同，归档验收结论以上述例外为准，不因归档目录存在而宣称这些目标全部达成。本次不修改 Agent 业务逻辑、预算、模型或正式数据，不部署生产环境。

- 例外归档执行结果：已通过 OpenSpec CLI 正常同步并归档到 `2026-09-18-fix-production-agent-finalization-phases`，未跳过规格或验证。两个主规格分别新增 24、1 条要求，逐块核对完整一致；两项未完成任务保持未勾选。归档后严格校验 63 项通过（active change 数量减少一项），并清理 CLI 生成的末尾多余空行。
