...
- 当前人工反馈支持清理后主要对话与 Web 操作正常。

## 归档同步（2026-09-16）

- 用户已明确批准归档、合并及推送；不操作阿里云部署。
- 归档预检要求保留修改需求的历史场景名称，并禁止生成空规格。同步前补齐 4 个场景入口的退役后描述，以及 5 份旧规格的历史只读兼容要求；均表达已批准的退役和保留边界，不修改业务代码。
- 已通过 CLI 同步 7 份主规格并归档至 `2026-09-16-retire-legacy-knowledge-agent-orchestration`。旧执行要求退役，历史数据、数据库字段和权限边界继续保留。
- CLI 对 REMOVED 区块缺少场景及 delta 数量较多给出非阻断警告；没有跳过校验或规格同步。归档后再次进行全库严格校验。
