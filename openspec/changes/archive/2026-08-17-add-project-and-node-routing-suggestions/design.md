...
- 目录推荐可重跑：项目内来源处理完即路由，全局来源自动归属项目后路由，修改项目重新路由。
...
`Source` 增加 `recommended_project_id`（可空，无外键，仅建议）与 `project_recommendation_reason`（可空文本）。只在全局收集箱来源处理时生成；项目内来源不生成、直接使用当前项目。推荐明确时自动写 `project_id`（不再要求用户确认），推荐字段保留审计。
...
- 全局来源：AI 推荐明确时自动归属项目，并在同一处理任务内同步路由；用户手动修改项目时同步触发重路由。
- 修改项目后：清除该来源候选旧推荐并重新路由，不复制候选或 Entry。

取舍：自动归属路径在 Worker 内执行，不阻塞请求；手动修改项目触发的重路由在 PATCH 内同步执行，离线/演示模型即时返回，真实 Provider 可能有数秒延迟，后续可异步化。
...
确认台目录下拉预填 `recommended_node_id`；`recommended` 直接高亮「采纳」；`needs_review` 展示主建议/备选/理由；`no_suitable` 不预填。项目归属由 AI 推荐后自动完成（来源级），确认台只处理候选内容/类型/目录。
...
- [路由是额外 AI 调用，增加处理耗时] → 项目内来源与自动归属的全局来源在 Worker 内串行；手动修改项目触发的重路由同步阻塞 PATCH，离线即时、真实 Provider 可能有数秒延迟，后续可异步化。
...
- 手动修改项目触发的重路由异步化留待后续，具体复用 Worker 队列模式确定。
