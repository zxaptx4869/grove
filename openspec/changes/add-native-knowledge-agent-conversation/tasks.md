...
- [x] 5.1 用原生组件替换默认对话页的禁用占位态：无历史时进入本地 draft，有历史时恢复最近活动对话；首次成功发送才创建服务端 Conversation，创建成功但提交未知时复用既有 conversation_id。
- [x] 5.2 实现历史 Bottom Sheet，展示标题、Workspace/项目范围、最近活动、活动主题与最新 Run 状态；支持选择既有对话和新建，不加入重命名、删除、搜索或置顶。
- [x] 5.3 实现最近消息恢复与向上加载更早消息，按 id 去重并 prepend，保持用户当前滚动位置；范围事件作为居中分隔条，历史回答继续显示各自 Run 范围快照。
- [x] 5.4 顶栏范围只提供当前 Workspace「全部知识」与项目；draft 先更新本地创建参数，既有对话调用 PATCH；相同范围不发请求/不产事件，活动 Run 时禁用并在服务端 409 后 refetch。
- [x] 5.5 对话头部展示当前活动主题和工作集 Entry 数，但不展开条目或支持单项移除；为 empty、loading、分页错误、范围错误和恢复状态补充组件测试并运行移动 test/lint/typecheck，随后完成一次本地提交。

## 6. 输入、模式与幂等提交

- [x] 6.1 实现真实系统键盘输入、发送按钮、可编辑建议问题和提交中的可访问状态；空文本不可发送，长文本与键盘不得遮挡 thread、composer 或底栏。
- [x] 6.2 实现低频模式 Sheet：理解方式 auto/继续当前主题/新话题，回答方式 auto/快速回答/深度查找；仅非默认值显示可移除 chip，收到确定提交响应后恢复 auto。
- [x] 6.3 将每次逻辑提交绑定稳定 `client_message_id`：结果未知的网络重试复用原键，终态 failed 的重新提问创建新键；201/幂等 200 均去重合并服务端消息和 Run。
- [x] 6.4 活动 Run 返回 409 时拉取会话最新 Run，不创建第二个本地任务；为重复点击、超时后重试、创建成功提交失败、幂等重放与模式参数补充测试，运行移动 test/lint/typecheck 后完成一次本地提交。

## 7. Run 过程、取消与前后台恢复

- [x] 7.1 将服务端 waiting/processing/current_step 映射为准备、理解问题、选择回答方式、检索正式知识、读取 Entry、核验证据、深度查找第 N 轮、综合回答等有限过程文案，不显示 controller reason 或隐藏推理。
- [x] 7.2 仅在 AppState active 且 Run 未终态时轮询；进入后台停止本地轮询，回到前台、重新登录或重新打开会话时立即 refetch，终态后停止。
- [x] 7.3 实现活动 Run 取消与「正在取消」过渡态，等待服务端终态；轮询网络失败保留已知 Run 并提供手动重试，不重新提交原问题。
- [x] 7.4 为前台/后台切换、重启恢复、终态停止、取消竞态、轮询失败与 409 恢复补充 hook/组件测试，运行移动 test/lint/typecheck 后完成一次本地提交。

## 8. 回答、引用与调查结果

- [x] 8.1 以结构化 `answer.status` 实现 completed、partial、insufficient、failed、clarification 回答卡；Run 完成状态不得掩盖知识不足，fallback 仅显示面向用户的阶段影响与结果。
- [x] 8.2 实现调查摘要「深度查找 · N 轮 / M 次查询」、停止原因，以及可展开的 coverage、gaps、conflicts；不展示完整 Query 账本、provider/model 原文或调试字段。
- [x] 8.3 在回答正文下展示真实 citation 来源条，点击打开原生 Bottom Sheet，分区显示 Entry、项目/目录、Source 标题和本次 Run 的原文快照；不伪造段落级内联引用。
- [x] 8.4 冲突卡展示 citation_a/citation_b 双方原文与归属；历史快照和当前知识跳转结果必须分区标注，对象已删除时仍保留快照且禁用当前对象入口。
- [x] 8.5 为五种回答状态、降级、不同项目引用、双边冲突、长原文、对象删除与 Sheet 交互补充测试，运行移动 test/lint/typecheck 后完成一次本地提交。
...
- [x] 9.2 在 390×844 主视口及 360×800、412×915 扩展视口走查空态、短/长对话、键盘展开、历史、范围、模式、过程、回答、引用与冲突；不得出现横向滚动、内容截断、顶栏/composer/底栏/系统键盘互相遮挡。（原生三视口/系统键盘依赖真机，当前以 Web 预览 + 程序化布局检查替代，缺口记录于 validation.md）
- [x] 9.3 校验安全区、动态字体/文本缩放、44×44 触控目标、读屏标签、焦点顺序、颜色对比和 reduce-motion；记录 iOS 与 Android 的实际差异，不用伪键盘或网页截图替代原生验收。（安全区/动态字体/44pt/reduce-motion 的原生验收待真机补齐，记录于 validation.md）
