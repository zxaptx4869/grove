...
## ADDED Requirement: 收尾句柄类型必须严格隔离
项目列表、Entry 结果集合、目录结果和其他结果句柄 MUST 按服务端记录类型分别校验；finalizer 不得猜测、替换或重新组织句柄。结构化列表由程序按句柄类型渲染，项目列表不得被当作 Entry 列表。

## ADDED Requirement: 精简候选正文允许程序补齐最小边界
用户只要求输出修改后候选版本时，系统 MUST 优先使用已保存候选稿或纯文本结果；finalizer 不得因缺少原记录章节而拒绝。程序 MUST 自动补齐尚未写入及模型补充不属于 Source 原文的最小边界，并保持候选稿只读。

#### Scenario: 项目列表句柄不混用
- **WHEN** finalizer 返回项目列表结果句柄
- **THEN** 程序按 `projects` 类型渲染项目列表，不按 Entry 列表校验或解析
