...
实验统一循环 MUST 从回答块、工具事件和程序侧对象状态重建历史，而不是把完整渲染回答直接写回模型上下文。完整聊天记录 MUST 保留全部用户原话与对话顺序；模型上下文 MUST 保留近期有界回答、当前编辑对象和用户决定、必要的授权列表 Entry ID/标题/顺序/范围及关系状态、当前未完成步骤，不得反复加入旧失败文案；已展开的 Entry 正文和 Evidence 原文 MUST 仅保存在程序侧，不得随历史逐轮重复。单轮回答摘要与整体历史 MUST 使用确定性大小边界，不得调用摘要模型；必要上下文经压缩仍超过硬上限时 MUST 不派发并准确说明限制。
...
#### Scenario: 否定的并列完成状态不应阻断候选稿
- **WHEN** 候选稿已满足原有边界，并说明“不代表已核验或已写入正式条目”
- **THEN** 普通输出及无工具收尾均不得将该否定声明判为实际写入；已保存的失败稿经原有权限复验后继续时，不重复搜索和读取

#### Scenario: 否定不能掩盖后续肯定声明
- **WHEN** 候选稿先说明未写入或否定完成状态，随后又声明“已写入”或“已经更新了正式记录”
- **THEN** 每一处完成声明均被检查，否定不能跨越转折、正文或新主语，回答仍被拒绝且正式记录不被修改
...
系统 MAY 确定性忽略 text 块最多 500 字的可选备注，也 MAY 在普通求解或 finalizer 的唯一文本响应内容与引用完整时，于格式重试之前将已知旧式 `answer_summary + completion` 外壳确定性转换为 `DialogueAnswer`。该转换 MUST 仅接受非空有界叙述与受支持引用类型，MUST 保留 Provider 原始响应和兼容事件，MUST 丢弃模型自报的完成状态，并 MUST 将转换结果交给既有当前轮句柄授权、类型、来源和 Workspace 校验。系统 MUST 继续拒绝其他错误顶层结构、错误句柄类型、历史结果、伪造引用及未授权或跨 Workspace 材料。Finalizer MUST 被明确要求只输出 `DialogueAnswer.blocks`，不得通过增加模型重试兼容非法输出。
...
#### Scenario: 普通 Agent 首次生成旧式候选稿
- **WHEN** 普通 Agent 首次响应以受支持旧式外壳生成包含未写入状态、原记录内容、新增建议、修改后版本和来源边界的完整候选稿，且没有资料工具调用
- **THEN** 程序在框架格式重试前转换并校验该回答，本轮不因纯格式偏差派发第二次正常请求或转入 finalizer，候选内容仍不写入正式 Entry
...
### Requirement: 授权结果统一展示
系统 MUST 按当前轮合法可展示结果的真实类型决定项目、统计、目录和 Entry 展示。模型只选择 result_handle 和解释，旧 list/statistic 展示声明不得让合法结果失败；Evidence 仍使用独立当前轮来源句柄校验，不得自动转换。前端 MUST 渲染真实项目 name 与统计桶（包含零值项目），不得将项目当 Entry。

#### Scenario: 项目和统计展示
- **WHEN** 模型选择项目句柄或把成功分组统计声明为 list
- **THEN** 服务端分别投影项目列表和统计块，前端显示真实项目名与包括空项目的分组数量

#### Scenario: 非法或过期引用
- **WHEN** result_handle 不存在、非当前轮、不可展示或属于 Evidence
- **THEN** 程序拒绝而不猜测替代句柄、不授予权限

### Requirement: 精简候选正文允许程序补齐最小边界
用户只要求输出修改后候选版本时，系统 MUST 优先使用已保存候选稿或纯文本结果；finalizer 不得因缺少原记录章节而拒绝。程序 MUST 自动补齐尚未写入及模型补充不属于 Source 原文的最小边界，并保持候选稿只读。

#### Scenario: 精简候选稿由程序补边界
- **WHEN** finalizer 只返回修改后候选正文
- **THEN** 程序保留正文并补齐尚未写入与 Source 原文边界，状态不因缺少原记录章节而失败

### Requirement: 完整请求预算内压缩历史
系统 MUST 保留完整聊天记录，但模型请求只使用近期摘要、当前对象、最新候选稿、用户决定和必要结果摘要。系统 MUST 在包含指令和工具定义的完整请求派发前确定性裁剪旧历史，避免重复错误提示和工具轨迹累积；MUST NOT 提高 9000/12000 或其他预算，不使用摘要模型。当前问题、工具结果和已保存决定不可静默丢弃，必要信息仍放不下时 MUST 明确停止。

#### Scenario: 长历史后短追问恢复派发
- **WHEN** 完整历史足以使未压缩请求超过 12000，用户发起短追问或新话题
- **THEN** 实际派发请求满足硬上限且保留必要对象与决定，完整历史仍保存，失败提示不重复积累

#### Scenario: 必要上下文超限
- **WHEN** 移除可裁剪历史后当前问题、编辑决定和工具定义仍超过上限
- **THEN** 请求不派发并明确记录输入边界，不裁掉关键约束

### Requirement: 当前编辑任务独立保存
系统 MUST 将当前已授权 Entry、候选稿全文和用户决定独立于最新查询结果保存。现有 Agent 可选择编辑、挂起或恢复，不新增意图判断模型。进入编辑或恢复 MUST 复验权限、对象及 Source 关系；候选与精简始终使用所选对象。查询其他主题 MUST 不引入旧候选校验，失败 continuation 与可恢复原稿 MUST 分离。

#### Scenario: 同一对象连续修改
- **WHEN** 用户打开第一条 ENF，要求补充并生成候选，再要求精简
- **THEN** 编辑任务保持该 Entry、最新版稿件与用户决定，普通成功不能删除稿件，也不扩展为整套知识

#### Scenario: 切换主题再返回
- **WHEN** 用户查询其他主题后明确回到原稿
- **THEN** 新查询不承担旧候选约束；原稿仅在复验通过后恢复，越权或变更材料不可恢复，continuation 不串入其他任务

#### Scenario: 已选对象的普通补充讨论
- **WHEN** 用户已打开第三条 NAF 后问“你觉得内容上还有哪些可补充的呢”，Agent 选择当前对象讨论
- **THEN** 程序复验该 Entry 后直接交给无工具回答器，保留该对象与用户决定，不自动搜索或展开整套记录，不强制候选稿章节

#### Scenario: 当前对象讨论收尾失败后继续
- **WHEN** 当前对象讨论收尾失败，用户下一轮继续
- **THEN** 只重新回答同一原问题，恢复已复验的 Entry 范围和讨论模式，不重复搜索、不将其他记录替换为修改目标

#### Scenario: 内部候选不进入收尾材料
- **WHEN** 搜索曾返回间接或不相关记录但授权集合已排除它们
- **THEN** finalizer 请求不包含其标题或正文，仍严格拒绝模型自行输出被排除标题

### Requirement: 最小声明不得重复或污染结构化查询
程序 MUST 将候选边界中的空白差异视为同一声明，不重复追加等价的程序尾注。程序 MUST 保留候选正文与只读/来源边界。纯统计、项目和目录结果 SHOULD 只说明范围、过滤和完整性，不附带与问题无关的官方交叉验证模板话。

#### Scenario: 紧邻 Source原文 的声明
- **WHEN** 候选稿已经说明“模型补充不属于Source原文”
- **THEN** 不再追加“模型补充不属于 Source 原文”，且正文和未写入边界保留

#### Scenario: 统计范围与无关模板句共存
- **WHEN** 纯结构化查询回答含范围说明及无关交叉验证模板句
- **THEN** 保留结果、范围、过滤和完整性，仅清除无关模板句，不影响混合来源核验回答

### Requirement: 完整零值计数覆盖同范围重复列表动作
系统 MUST 仅依据当前轮合法、完整、精确的零值总计，返回同项目、目录范围和类型过滤的空列表结论，停止该范围的重复读取，MUST NOT 截断其他范围目标。搜索空结果、分组零桶及不完整计数不能作为零值证明。不存在同范围零值证明时，目录与语义搜索的既有非法组合校验 MUST 保持。

#### Scenario: 目录本身为零后重复列举
- **WHEN** 某目录 direct 范围完整计数为 0，模型又要求列出该范围记录，甚至附带多余目录语义词
- **THEN** 程序仅复用该范围的空集合结论，不执行重复资料查询、不调用语义搜索，不产生由该冗余动作引起的 partial_completed

#### Scenario: 零值不能外推到其他范围
- **WHEN** 零值来自不同项目、不同目录、不同 direct/subtree 范围、不同类型过滤、历史结果或不完整结果
- **THEN** 后续列表仍走原有授权与查询路径，不能复用零值或提前结束其他目标

### Requirement: 知识协作助手身份与自然表达
普通 Agent 与无资料工具 finalizer MUST 使用一致的“知林 Grove 的知识协作助手”身份，以帮助用户查找、理解、比较和完善知识为职责。回答 SHOULD 先回应当前问题，使用日常语言并按需解释；不主动向用户罗列内部类型、句柄或收尾阶段。不改变权限、资料依据与候选只读边界。

#### Scenario: 自我介绍或能力说明无需资料查询
- **WHEN** 用户询问身份、能力或进行无需资料的普通交流
- **THEN** 指令要求直接用现有 final_result 交付简短 text 回答，不为身份新增关键词路由，不将无需资料工具误解为无需结构化交付，不承诺联网或自动修改正式记录

#### Scenario: 收尾保持同一身份与候选表达
- **WHEN** 当前回答交给独立 finalizer，或用户要求补充、改写知识
- **THEN** 收尾保持同一用户身份；候选优先给出可阅读正文，必要时自然交代现有内容、补充判断、未写入和来源边界，不强制固定章节，用户决定是否采纳

#### Scenario: 身份指令不能绕过输出协议
- **WHEN** 模型仍返回协议之外的普通文本、非法句柄或越权资料
- **THEN** 原有校验和预算边界继续生效，不因身份调整自动接受原始文本或增加重试次数；失败不能静默记为成功
