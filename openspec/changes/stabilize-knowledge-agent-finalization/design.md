...
3. 编辑上下文与最近查询结果、失败 continuation 分开。成功 open_list_item 确立当前对象；现有 Agent 使用一个受限的编辑上下文工具选择修改、挂起或恢复，不增加判断模型或关键词分支。进入编辑前复验 Workspace 成员、授权 Entry、Source 关系及指纹，保存用户原始决定而非模型重写的约束。新查询挂起当前编辑；明确返回原稿通过工具复验再激活。候选成功保存全文，下一轮不因普通成功清理而丢稿；失败恢复仍使用原 finalize-only 机制。
