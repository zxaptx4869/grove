...
候选修改的交付信号增加与修改动作组合使用的“输出”；当用户明确说“只/就输出补充后的知识内容，其他不用”时，程序标记为精简候选稿：只允许一个 text 块，包含候选正文、尚未写入声明和简短来源边界，不输出 list、statistic 或 evidence 块，从而避免渲染器展开完整 Source 原文。普通候选稿仍保留现有内容与新增建议的区分及来源边界，可自然融入候选正文，不强制固定分区。
...
### 193809 会话：候选完成声明的否定范围

普通输出和独立 finalizer 继续共用候选校验。替换首次命中和前四字窗口：逐处检查既有完成声明，仅将明确否定直接支配的完成状态及紧邻的并列状态视为否定；例如“不代表已核验或已写入”。除并列顿号外的分句标点、换行、转折、其他正文及新主语不能让否定扩散到后续肯定声明；嵌套否定保守拒绝。未识别的复杂表达仍保守拒绝，不引入语义分类模型。保留未写入、来源边界和既有候选结构要求；失败稿仍按现有 continuation 复验权限后无工具收尾。

### 知识协作助手身份与交付协议

复用单份角色指令，分别组合进普通 Agent 和独立 finalizer 的系统指令，保持查找、理解、比较、完善知识的用户身份一致；内部阶段名不作为自我介绍。表达先回应当前问题，沿用当前对象与用户决定，不每轮重复身份或免责声明，不把候选承诺为正式写入。候选专用指令去掉诱导固定分区的措辞，在正文中自然交代现有内容、补充判断与来源边界，原有输出校验不变。

身份与程序输出协议分开：自我介绍、能力说明及普通回应不必调用资料工具，但仍调用 final_result 输出 DialogueAnswer；final_result 是答案交付而非资料动作。主流程与收尾共享该说明，不按“你是谁”增加字符串分支或硬编码产品回答。原始普通文本不自动包装，也不绕过验证；本次降低格式误选概率，不能宣称已经消除所有原始文本格式失败。确定性模型只验证指令实际送达、合法答案完成、原有非法输出拒绝与预算边界；真实模型的身份表达和交付质量仍待用户验收。

### 215527 会话：语义查询去重边界

语义查询缓存只在当前轮生效；每轮开始清除上一轮的查询键，用户下一轮明确追问或新主题查询不被旧结果阻断。同一轮仍可跨搜索表面复用已成功返回的 completed、empty 或 limited 结果，error 与 not_executed 不写入缓存。缓存只防止同轮等价动作重复，不改变结果展示、相关性授权和 Workspace 校验。

## 104716 会话实施设计（2026-09-14）

- 在实验目录内增加项目介绍读取模块；查询 Workspace 成员及项目归属，读取 Project.description 与 ProjectContext 已存快照。复用现有字段组装逻辑而不调用带 get-or-create / schedule-refresh 的读取服务。明确缺失、pending、failed、is_fallback、generated_at、version 与用户纠正优先级。不新增迁移或正式 API。
- 实验工具返回独立 project_context 结果类型；作为自然语言概括依据。误用 result 引用时程序投影为文本而非列表；原非法、过期、不可展示句柄仍拒绝。保存结果用于收尾，continuation 复验项目与快照，权限或数据变化时拒绝恢复。
- 替换项目枚举与项目介绍混用的提示；材料依据不等于展示列表。表达追问允许直接重述历史答案，不能声称本轮重新查询；如需重新使用项目快照只读取该项目，不追加统计、目录与语义搜索。
- 历史仅保护最近集合的句柄、顺序、对象名称/ID 和必要目录信息，统计条件和完整桶留在程序状态；近期叙述继续标为可裁剪。最终回答只接收本轮材料，历史保留用户原话与指代线索，不携带旧统计正文。
- 用同一未授权候选检测供校验、收尾和终态使用。预算切断授权时生成程序级部分完成说明，保留当前轮可展示结果，不让模型把未授权误说成未检索；不新增授权模型或绕过三分规则。无可恢复授权 continuation 时明确重新查询，不伪造“只需重新回答”的恢复能力。
- 确定性测试覆盖实际模型请求/工具路径、数据库无副作用、权限/跨 Workspace、项目数据变化后的恢复失败、长历史查询成功与必要上下文超限后的部分完成。真实语言选择仍待工作台验收。

实现核对发现 Pydantic AI 合并相邻合成消息时会丢弃消息级 metadata，导致可裁剪标记丢失。改为程序生成的 TextPart 级标记，完整请求压缩只删除这些历史叙述部分，保留同一合并消息里的对象、当前问题、工具返回和指令；不靠文字关键词识别历史。
