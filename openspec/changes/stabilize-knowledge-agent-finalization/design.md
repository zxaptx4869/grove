...
普通 Agent 的候选稿输出校验失败时，程序在进入重试或 finalize 前保存结构化 `DialogueAnswer` 文本及缺口，不把它只留在渲染字符串或日志中。finalize 动态提示和装配历史优先携带该候选稿，仅允许做最小整理；收尾响应若尝试资料工具仍在模型边界被拦截，不执行工具、不引入新的 Evidence，也不丢弃原稿。失败展示只渲染可安全验证的 text/insufficient 块，并明确“候选稿尚未通过最终边界校验”。

候选稿失败创建 `candidate_draft` continuation，保存原始问题、候选文本、当前轮已授权记录与来源材料及其指纹；公开快照只显示材料句柄和摘要。用户明确“继续”时复验 Workspace、Entry、Source、来源关系和材料版本后，仅调用无资料 finalizer 整理候选稿，不重复搜索、Entry/Evidence 读取或引入历史句柄。候选稿与材料失效时清除续执行并要求重新核验；成功完成后清理候选稿状态。
