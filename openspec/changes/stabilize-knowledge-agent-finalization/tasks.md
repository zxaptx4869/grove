...
- [ ] 2.11 在 finalizer 响应边界受限转换已知 `answer_summary + completion` 外壳，忽略模型完成声明并复用既有引用与 Workspace 校验
...
- [ ] 3.7 使用最新 F4 星真实载荷覆盖旧外壳成功转换、伪造或历史引用拒绝、未知结构拒绝、单模型请求与 finalizer 零资料工具执行
