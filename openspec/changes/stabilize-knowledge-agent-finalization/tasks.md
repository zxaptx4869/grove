...
- [ ] 2.8 为 DeepSeek V4 普通求解与 finalizer 显式关闭思考模式，并保持模型名与全部冻结预算不变
- [ ] 2.9 将 `can_continue` 收敛为真实 continuation，拦截无 continuation 的“继续”且不调用模型或资料工具
- [ ] 2.10 兼容 text 块无害备注而继续拒绝错误句柄与越权引用，实现只输出内容的精简候选稿且不展开 Evidence
...
- [ ] 3.6 补充 V4 非思考设置、三类真实收尾非法输出、continuation 真值、孤立继续零调用及精简候选稿渲染回归
