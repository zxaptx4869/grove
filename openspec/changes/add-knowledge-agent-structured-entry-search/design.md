...
当用户认为自动结果形态不合适时，移动端在答案卡/结果卡提供“列出相关知识 / 改为综合回答”纠正按钮；点击后以新的 `client_message_id` 和相反的显式 `result_mode` 直接重新提交原问题（用户显式点击发起，不是后台静默行为），保留原消息的上下文模式以避免追问被误当新话题。新 Run 独立创建，历史结果不被修改；对话存在活动 Run 时按冲突提示处理。如需修改措辞，用户可先编辑 Composer 再自行发送。
