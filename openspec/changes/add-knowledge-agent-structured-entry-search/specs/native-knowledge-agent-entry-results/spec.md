...
原生 App MUST 在“本次提问设置”中增加 `自动 / 综合回答 / 知识列表` 结果形态选择，并将非自动选择作为可移除 mode chip 展示；设置只作用于下一条消息，发送成功后恢复自动。自动路由结果与用户意图不符时，界面 MUST 提供将原问题填回 Composer 并预设相反结果形态的纠正动作，不得静默重新提交。
...
- **THEN** App 将原问题放回 Composer、预设 `result_mode=answer` 并等待用户确认发送，不创建重复 Run

#### Scenario: 把回答改为知识列表
- **WHEN** 用户在合格的综合回答中点击“列出相关知识”
- **THEN** App 将原问题放回 Composer、预设 `result_mode=entries` 并等待用户确认发送
