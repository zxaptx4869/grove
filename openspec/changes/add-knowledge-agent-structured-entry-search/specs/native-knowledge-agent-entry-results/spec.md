...
原生 App MUST 在“本次提问设置”中增加 `自动 / 综合回答 / 知识列表` 结果形态选择，并将非自动选择作为可移除 mode chip 展示；设置只作用于下一条消息，发送成功后恢复自动。自动路由结果与用户意图不符时，界面 MUST 提供“改为综合回答 / 列出相关知识”纠正动作；点击后 App 以新的 `client_message_id` 和相反的显式 `result_mode` 直接重新提交原问题（用户显式点击发起，不属于后台静默提交），不得修改历史 Run 或复用旧消息标识。
