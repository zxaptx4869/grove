...
- **THEN** 页头提供「思维导图」入口，点击进入项目级思维导图阅读视图（`view=mindmap`），知识空间工作台不显示静态或不可用的思维导图内容

#### Scenario: 节点定位参数
- **WHEN** 用户从思维导图点击「在知识空间中打开」且链接携带 `node` 参数
- **THEN** 知识空间加载后选中该参数对应节点，并展示其路径、名称、说明与知识（或空态）

#### Scenario: 未实现知识模式不冒充可用
- **WHEN** 除已实现的思维导图外，某项知识模式尚无当前 OpenSpec 实现
