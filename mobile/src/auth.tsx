...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const clear = async () => { await SecureStore.deleteItemAsync(key); setToken(null); setMe(null); };
  useEffect(() => { void (async () => { const saved = await SecureStore.getItemAsync(key); if (saved) try { setMe(await getMe(saved)); setToken(saved); } catch (error) { if ((error as { status?: number }).status === 401) await clear(); } setLoading(false); })(); }, []);
  const value = useMemo<Auth>(() => ({ token, me, loading, signIn: async (username, password, register) => { const result = await mobileLogin(username, password, register); await SecureStore.setItemAsync(key, result.token); setToken(result.token); setMe(await getMe(result.token)); }, signOut: async () => { if (token) try { await mobileLogout(token); } finally { await clear(); } } }), [token, me, loading]);
