...
import { PermissionNotice } from "@/src/capture/components/PermissionNotice";
import { ProjectSheet } from "@/src/capture/components/ProjectSheet";
import { SubmitStatus, type CaptureSession } from "@/src/capture/components/SubmitStatus";
...
export function CaptureScreen() {
  ...
  const busy = preparing || Boolean(session?.running);
  ...
  async function prepareFromAssets(assets: PickedAsset[], kind: CaptureKind) {
    ...
    try {
      ...
      setDraft(emptyDraft(kind, images));
    } catch {
      ...
      >
        ...
        {draft ? (
          <CaptureForm
            ...
            error={draftError}
            showSubmit={!session}
            onChangeText={(value) => setDraft((previous) => (previous ? { ...previous, text: value } : previous))}
            onChangeNote={(value) => setDraft((previous) => (previous ? { ...previous, note: value } : previous))}
            onRemoveImage={(index) =>
              setDraft((previous) =>
                previous
