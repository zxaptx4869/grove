...
核心可点击场景从「对话」页的建议问题开始，先覆盖项目范围、知识检索、Entry 与 Source 引用、冲突与知识不足，然后可连续走通四类只读追问：解释已有结论、查看 Source 原文、增加新条件后补检索、寻找反例与不同观点。「整理成知识」作为其中一个后续动作，继续保留歧义澄清、可编辑草稿、完整差异、操作确认、执行回执与撤销。其余三个栏目提供轻量可点击状态，用于验证采集、长任务恢复、失败重试、审计和对象查看。
