...
候选在本原型中始终只读：详情页不提供确认、拒绝、写入或归档操作，也不写去路说明，只读性由「没有任何写入操作」体现；详情页底部的「改归属 / 删除」只作用于来源本身，不触达候选。本轮不做录音、网页链接、系统分享、连拍扫描、独立「全部来源」页、列表搜索与状态筛选，也不用百分比进度条表达处理中。顶部虚线区域是原型专用的状态控制，正式实现不含该区域。走查证据见 `artifacts/mobile-capture-prototype/`。
