...
- [x] 项目说明可选字段；项目生命周期增加进行中、暂停、已完成、已归档。
...
- [x] 按 Source 审阅，逐条确认、编辑、拒绝和暂缓。
- [x] 当前 Source 内多选确认或拒绝。
- [x] 确认后创建 Entry，并保留 Candidate 与 Source 证据关系。
- [x] Entry 卡片与列表两种视图。
- [x] 按目录浏览，区分节点直接知识与子树知识。
- [x] Entry 详情、来源查看、后续编辑和移动目录节点。
- [x] 项目内与全局关键词搜索。
- [x] Processing Task 进度、失败原因和幂等重试。
- [x] 小屏访问统一显示电脑访问提示，不加载业务工作台或提供继续访问入口。

### P0-B：整理效率闭环

- [x] 全局收集箱中 AI 推荐 Source 所属项目。
- [x] 每条 Candidate 推荐真实目录 node_id、备选和理由。
- [x] 推荐明确、需要确认、暂无合适位置三档路由状态。
- [x] 确认 Candidate 时一次接受内容、类型、项目和目录。
- [x] 新增节点并归档的原子操作。
- [x] 跨 Source 批量处理视图。
- [x] 按推荐目录分组确认、批量改目录和批量拒绝。
- [x] 高风险 Candidate 自动退出批量快审。
- [x] 与已有 Entry 的基本相似检索。
...
- [x] Directory Agent 从零起草目录并进行必要澄清。
- [x] 可视化 Directory Draft、内联编辑和确认应用。
- [x] 与 AI 对话调整草稿，使用结构化增量操作。
- [x] 对任意节点进行 AI 拓展。
- [x] 对现有目录提出新增、改名、移动、更新说明和建议删除。
- [x] 目录差异展示、受影响 Entry 数量与安全合并。
- [x] 语义搜索和相似知识推荐。
- [x] Reader Agent 节点范围与项目范围问答。
- [x] 回答引用 Entry 和 Source，知识不足与冲突可见。
- [x] 回答内容转 Candidate 的确认流程。
- [x] 思维导图目录浏览、聚焦、高亮和节点知识侧栏。
- [x] 知识全景视图：旭日图全局结构/知识密度浏览，与思维导图合并为单一「知识全景」入口并支持顶部切换与跨模式联动。
- [x] Entry 基础版本历史与 AI 修订建议。
- [x] Entry AI 修订建议结合知识库与外部知识（AI 补充可辨识、应用时以虚拟 Source 沉淀溯源）。
