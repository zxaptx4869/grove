...
from app.schemas.auth import LoginRequest, MobileAuthResponse, RegisterRequest, UserOut
...
async def _create_mobile_response(
    payload: RegisterRequest | LoginRequest, db: DbSession, *, register: bool
) -> MobileAuthResponse:
    """创建移动端会话，复用 Session 哈希存储且只向移动端返回原始 Token。"""
    if register:
        existing = (
    ...
    return MobileAuthResponse(
        user=UserOut(id=user.id, username=user.username, created_at=user.created_at), token=token
    )
...
@router.post(
    "/mobile/register", status_code=status.HTTP_201_CREATED, response_model=MobileAuthResponse
)
async def mobile_register(payload: RegisterRequest, db: DbSession) -> MobileAuthResponse:
    """原生移动端注册：返回仅可用于 Bearer 的不透明会话 Token。"""
    return await _create_mobile_response(payload, db, register=True)


@router.post("/mobile/login", response_model=MobileAuthResponse)
async def mobile_login(payload: LoginRequest, db: DbSession) -> MobileAuthResponse:
    """原生移动端登录：返回仅可用于 Bearer 的不透明会话 Token。"""
    return await _create_mobile_response(payload, db, register=False)
...
async def mobile_logout(
    ...
    """撤销当前 Bearer 会话；认证依赖已严格拒绝非 Bearer 请求。"""
    ...
    session = (
        await db.execute(
            select(Session).where(
                Session.token_hash == hash_session_token(token), Session.user_id == user.id
            )
        )
    ).scalar_one_or_none()
    if session is not None:
