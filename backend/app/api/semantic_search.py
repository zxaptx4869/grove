...
from app.services.entry import entry_eager_options, entry_out
...
def _to_response(results: list[tuple[Entry, str, bool]]) -> list[SemanticEntryOut]:
    """把语义检索结果组装为响应模型。"""
    return [
        SemanticEntryOut(
        ...
        for entry, reason, is_fallback in results
    ]
