...
RESULT_MODE_ROUTE_SYSTEM_PROMPT = (
    ...
    "选择 answer；"
    "\n"
    "3. 同一问题同时包含解释、知识库检索、统计、比较或建议中的"
    "两种及以上时，选择 answer；只有纯列表、纯筛选或用户只需要"
    "结构化结果集的统计才选择 entries；"
    "\n"
    "4. 拿不准、查询既有理解又有找对象成分时选择 answer，"
    "避免把「找到以后理解它」误判成纯对象列表；"
    "\n"
    "5. 只允许输出 answer 或 entries，不要输出其他模式；"
    "\n"
    "6. reason 用不超过 100 字说明判断依据。"
)
