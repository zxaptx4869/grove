...
核心基线额外固定整批 40 条用户消息、192 条模型审计记录的轮后停止阈值
和 1800 秒上限。模型审计阈值不是逐请求派发硬上限：一轮内可产生多条审计记录，
因此轮后数量可超过阈值；评测器会在下一轮消息派发前停止。这些是评测器的停止线，
不修改 Agent 单轮预算、重试或模型配置。
...
都不会自动把语义层改为通过。语义标注以 `(case_id, repeat, turn)` 唯一定位；
旧的单次格式可省略 `repeat`，但只有在目标唯一时才接受。新标注另存
`reviewed-report*.json` 和 `reviewed-summary*.md`，已有文件存在时生成唯一后缀，不覆盖原始证据或旧审阅。
...
实际展示对象的本轮引用，以及用户“不查知识库”的工具行为边界。统计、分组和项目枚举优先核对
正式 API 的公开结构化结果；纯文本只在完整匹配极窄、无歧义格式时才作为交付证据。
内部工具事实正确但无法确认公开交付时标记待审阅，不会判通过。
...
摘要分开“模型审计记录数”与“已派发记录数”；`not_dispatched` 只记入前者。
已派发阶段不一定返回 usage，摘要只汇总能取得的 provider usage，缺失数量另行标明；
