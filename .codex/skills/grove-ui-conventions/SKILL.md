...
- **探索性原型流程**：尚未进入 OpenSpec 的产品形态探索或独立可点击原型。不得宣称为正式能力；没有 change 时把基线、偏离和验收记录到原型说明与 `artifacts/`（本地证据目录，不入 Git），不伪造 `design.md`。原生 App、移动端或对话 Agent 原型必须读取 [原生移动端与对话 Agent 原型流程](references/native-mobile-agent-prototype-workflow.md)。

Web 任务明确要求“跟原型一致”“对齐设计稿”“还原页面”“检查颜色、间距、字体”或同义目标时，至少进入局部视觉流程，整个页面进入完整流程；原生端按对应流程保留视觉要求与用户验收职责。

记录规则：有 change 时在 `design.md` 写必要基线、偏离与验收结论，已有独立验收记录则引用；探索原型在对应原型说明记录，索引只保留链接；无 change 的局部修复在交付说明记录简短结论，必要截图沿用 `artifacts/` 等现有本地位置（不入 Git），不为此创建 change 或 design.md。后文所有基线与验收记录均遵循此规则。
