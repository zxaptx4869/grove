...
1. `docs/产品蓝图与功能优先级.md`：产品对象、流程、页面和优先级。
2. 当前 OpenSpec change 与 `openspec/specs/`：本次必须实现和明确不做的行为。
3. `frontend/src/index.css`：颜色、字号、圆角、阴影和动效令牌。
4. 相关页面与组件代码：现有接口和交互模式。

冲突时依次遵循 OpenSpec、产品蓝图、实际主题和现有代码。不要在本 skill 维护第二套组件 API 或令牌值；修改主题后再检查本 skill 是否仍准确。
